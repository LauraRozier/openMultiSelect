...
(function ($, window, document, undefined) {
  ...
  function Plugin(aElement, aOptions) {
    ...
    this.settings = $.extend(true, {}, $.fn.openMultiSelect.defaults, aOptions);
    /**
    ...
    this.items = ITEMS_DEFAULTS; // #endregion Fields
  ...
  $.extend(Plugin.prototype, {
    ...
    cbSelectItem: function (aEvent) {
      ...
      const $caller = $(aEvent.target).removeClass("oms-item-" + aEvent.data.settings.availableItemColor).addClass("oms-item-" + aEvent.data.settings.selectedItemColor).off("click").click(aEvent.data, aEvent.data.cbUnselectItem);
    ...
    cbUnselectItem: function (aEvent) {
      aEvent.preventDefault();
      aEvent.stopImmediatePropagation();
      const $caller = $(aEvent.target).removeClass("oms-item-" + aEvent.data.settings.selectedItemColor).addClass("oms-item-" + aEvent.data.settings.availableItemColor).off("click").click(aEvent.data, aEvent.data.cbSelectItem);
      aEvent.data.items.selected = aEvent.data.arrayRemove(aEvent.data.items.selected, $caller.val());
      aEvent.data.valueElement.val(JSON.stringify(aEvent.data.items.selected));
    ...
    cbSelectAll: function (aEvent) {
      aEvent.preventDefault();
      aEvent.stopImmediatePropagation();
      const items = aEvent.data.availableElement.find(".oms-list-body").find(".oms-item");
      $.each(items, function (index, value) {
        const $value = $(value);
        aEvent.data.items.selected.push($value.val());
        $value.removeClass("oms-item-" + aEvent.data.settings.availableItemColor).addClass("oms-item-" + aEvent.data.settings.selectedItemColor).off("click").click(aEvent.data, aEvent.data.cbUnselectItem).appendTo(aEvent.data.selectedElement.find(".oms-list-body"));
      });
      aEvent.data.valueElement.val(JSON.stringify(aEvent.data.items.selected));
    },
    ...
    cbUnselectAll: function (aEvent) {
      aEvent.preventDefault();
      aEvent.stopImmediatePropagation();
      const items = aEvent.data.selectedElement.find(".oms-list-body").find(".oms-item");
      $.each(items, function (index, value) {
        const $value = $(value);
        aEvent.data.items.selected = aEvent.data.arrayRemove(aEvent.data.items.selected, $value.val());
        $value.removeClass("oms-item-" + aEvent.data.settings.selectedItemColor).addClass("oms-item-" + aEvent.data.settings.availableItemColor).off("click").click(aEvent.data, aEvent.data.cbSelectItem).appendTo(aEvent.data.availableElement.find(".oms-list-body"));
      });
      aEvent.data.valueElement.val(JSON.stringify(aEvent.data.items.selected));
    },
    // #endregion Callbacks
    ...
    render: function () {
      ...
      $.each(this.items.available, function (index, value) {
        const listItem = $("<span />").attr({
          class: "oms-item"
        }).text(value.text).val(value.value);

        if (this.arrayContains(value.value, this.items.selected)) {
          seBody.append(listItem.addClass("oms-item-" + this.settings.selectedItemColor).click(this, this.cbUnselectItem));
        } else {
          aeBody.append(listItem.addClass("oms-item-" + this.settings.availableItemColor).click(this, this.cbSelectItem));
        }
      }.bind(this));
    },
    ...
    reloadAvailableItems: function (aUrl, aIndRedraw) {
      ...
      if (aUrl !== null) {
        if (this.settings.ajax === null) {
          this.settings.ajax = $.extend(true, {}, AJAX_DEFAULTS, {
            availableUrl: aUrl
          });
        } else {
      ...
      const ajaxSettings = $.extend(true, {}, AJAX_DEFAULTS, this.settings.ajax);
      this.items = ITEMS_DEFAULTS; // Reload using AJAX
    ...
    reloadItems: function (aAvailableUrl, aSelectedUrl, aIndRedraw) {
      aAvailableUrl = typeof aAvailableUrl !== "undefined" ? aAvailableUrl : null;
      aSelectedUrl = typeof aSelectedUrl !== "undefined" ? aSelectedUrl : null;
      aIndRedraw = typeof aIndRedraw !== "undefined" ? aIndRedraw : true;

      if (this.reloadAvailableItems(aAvailableUrl, false) && this.reloadSelectedItems(aSelectedUrl, false)) {
        if (aIndRedraw === true) this.render();
      }
    },
    ...
    init: function () {
      ...
      this.availableElement = $("<div />").attr({
        class: "oms-available-list"
      }).append($("<div />").attr({
        class: "oms-list-header oms-list-header-" + this.settings.headerColor
      }).text("Available Items"), $("<div />").attr({
        class: "oms-list-body"
      }));
      this.selectedElement = $("<div />").attr({
        class: "oms-selected-list"
      }).append($("<div />").attr({
        class: "oms-list-header oms-list-header-" + this.settings.headerColor
      }).text("Selected Items"), $("<div />").attr({
        class: "oms-list-body"
      }));
      this.buttonElement = $("<div />").attr({
        class: "oms-controls"
      }).append($("<button />").attr({
        class: "btn btn-block btn-" + this.settings.buttonColor + " mb-1"
      }).text("Reset Selection").click(this, this.cbResetSelection), $("<button />").attr({
        class: "btn btn-block btn-" + this.settings.buttonColor + " mt-3"
      }).text("Select All").click(this, this.cbSelectAll), $("<button />").attr({
        class: "btn btn-block btn-" + this.settings.buttonColor + " mt-3"
      }).text("Unselect All").click(this, this.cbUnselectAll));
      this.$element.append($("<div />").attr({
        class: "row mt-5 mb-4 clearfix"
      }).append(this.valueElement, this.availableElement, this.buttonElement, this.selectedElement));
  ...
  $.fn.openMultiSelect = function (aOptions) {
    ...
    if (typeof aOptions === "string" && aOptions[0] !== "_" && aOptions !== "init") {
      // Cache the method call to make it possible to return a value
      let returns;
      this.each(function (index, value) {
        const instance = $.data(value, PLUGIN_NAMESPACE_DATA);

        if (instance instanceof Plugin && typeof instance[aOptions] === "function") {
...
//# sourceMappingURL=openMultiSelect_compiled.js.map