...
; (function($, window, document, undefined) {
    ...
    function Plugin(aElement, aOptions) {
         ...
         * @var {Object} settings
         */
        this.settings = $.extend({}, $.fn[PLUGIN_NAMESPACE].defaults, aOptions);
        /**
    ...
    $.extend(Plugin.prototype, {
        ...
        cbSelectItem: function(aEvent) {
            ...
            aEvent.data.valueElement.val(
                JSON.stringify(aEvent.data.items.selected)
            );
            $caller.appendTo(
                aEvent.data.selectedElement.find(".oms-list-body")
            );
        },
        ...
        cbUnselectItem: function(aEvent) {
            ...
            $caller.appendTo(
                aEvent.data.availableElement.find(".oms-list-body")
            );
        },
         ...
         * available items. Pass `null` to keep the original url.
         * Default: `null`
         * @param {boolean} aIndRedraw Indicated that the element should be
         * redrawn on success. Default: `true`
         *
         * @author  Thimo Braker <thibmorozier@gmail.com>
         * @version 1.0.0
         */
        reloadAvailableItems: function(aUrl, aIndRedraw) {
            ...
            aIndRedraw = (typeof aIndRedraw !== "undefined")
                ? aIndRedraw
                : true;
            ...
            $.ajax({
                ...
                this.items.available = Object.assign(data);
         ...
         *
         * @param {string} aUrl Sets the new URL to use when reloading the
         * selected items. Pass `null` to keep the original url.
         * Default: `null`
         * @param {boolean} aIndRedraw Indicated that the element should be
         * redrawn on success. Default: `true`
         *
         ...
         * @method reloadItems
         *
         * @param {string} aAvailableUrl Sets the new URL to use when reloading
         * the available items. Pass `null` to keep the original url.
         * Default: `null`
         * @param {string} aSelectedUrl Sets the new URL to use when reloading
         * the selected items. Pass `null` to keep the original url.
         * Default: `null`
         * @param {boolean} aIndRedraw Indicated that the element should be
         * redrawn on success. Default: `true`
         *
         ...
         * @version 1.0.0
         */
        setAvailableItems: function(aAvailableItems, aIndRedraw) {
            aIndRedraw = (typeof aIndRedraw !== "undefined")
                ? aIndRedraw
                : true;

            this.items.available = Object.assign(aAvailableItems);
            if (aIndRedraw === true) this.render();
        },
         ...
         * @version 1.0.0
         */
        setSelectedItems: function(aSelectedItems, aIndRedraw) {
            aIndRedraw = (typeof aIndRedraw !== "undefined")
                ? aIndRedraw
                : true;

            this.items.initial = Array.from(aSelectedItems);
            this.items.selected = Array.from(aSelectedItems);
            if (aIndRedraw === true) this.render();
        },
        ...
        init: function() {
            ...
            this.buttonElement = $("<div />").attr({
                ...
                $("<button />").attr({
                    class: "btn btn-block btn-" + this.settings.buttonColor +
                    " mb-1",
        ...
        destroy: function() {
            this.$element.each(function(index, value) {
                value.clearItems();
                $(value).removeData(PLUGIN_NAMESPACE_DATA);
            });
        },
        ...
        if (aOptions === undefined || typeof aOptions === "object") {
            return this.each(function(index, value) {
                if (!$.data(value, PLUGIN_NAMESPACE_DATA)) {
                    $.data(
                        value,
                        PLUGIN_NAMESPACE_DATA,
                        new Plugin(value, aOptions)
                    );
                }
            });
        }
